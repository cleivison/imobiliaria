...
function CadastrarInquilino(props){
    ...
    {
        nome: state? state.inquilino.nome : '',
        cpf: state? state.inquilino.cpf : '',
        rg: state? state.inquilino.rg : '',
        dataNascimento: state? state.inquilino.dataNascimento : '',
        rua: state? state.inquilino.Endereco.rua :'',
        bairro: state? state.inquilino.Endereco.bairro :'',
        cidade: state? state.inquilino.Endereco.cidade :'Uberaba',
        estado: state? state.inquilino.Endereco.estado :'MG',
        numero: state? state.inquilino.Endereco.numero :'',
        referencia: state? state.inquilino.Endereco.referencia :'',
        cep: state? state.inquilino.Endereco.cep :'',
        telefone: state? state.inquilino.Endereco.telefone :''
    }
    );
    ...
    useEffect(() => {
        async function getItem(){
            const res = await api.get('inquilino/'+params.id);
            ...
            if(res.status === 200){
                ...
                history.push('/dashboard/inquilinos');
            }
        }
    ...
    const handleSubmit = async (e) => {
        ...
        if(!isLoading){
            ...
            if(params.id === undefined){
                res = await api.post('inquilino', inquilino);
                mensagem = 'Proprietário cadastrado com sucesso!';
            }else{
                res = await api.put('inquilino/'+params.id, inquilino);
        ...
        {message.map((msg, key) => (
         ...
         <form>
            <Row justifyContent="flex-end" style={{paddingBottom: '1rem', paddingRight: '1rem'}}>
                <Button onClick={() => history.push('/dashboard/inquilinos')} type="primary" icon="rollback" size="large">Voltar</Button>
            </Row>
            <Row>
                <Col>
...
export default CadastrarInquilino;
