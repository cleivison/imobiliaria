...
function Proprietario () {
    ...
    useEffect(() => {
        async function getImoveis() {
            ...
            //console.log(res);
            if(res.status === 200){
                setTablePagination({data: res.data.proprietario, loading: false, pagination: {...tablePagination.pagination, total: res.data.count}});
            }else{
    ...
    const columns = [
        ...
        },
        {
          ...
          render: (proprietario) => (
            <ActionButtons>
              <a href={`proprietarios/cadastrar/${proprietario.id}`} onClick={(e) => {
                e.preventDefault();
                history.push(`proprietarios/cadastrar/${proprietario.id}`,{
                    proprietario: proprietario
                });
              }}>Editar</a>
              <Divider type="vertical" />
              <Popconfirm
                title={`Deseja realmente deletar o proprietário de cpf ${proprietario.cpf}?`}
                onConfirm={() => {handleDelete(proprietario.id)}}
    ...
    const handleDelete = async (id) => {
        const res = await api.delete(`proprietario/${id}`);
        if(res.status === 200){
